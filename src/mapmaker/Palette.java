...
public class Palette extends GridPane {
    ...
    private void constructPalette(Image source, Tiled[] cells, int rows, int columns) {
        for (int x = 0; x < cells.length; x++) {
            int row = (x % (rows * columns)) / columns, col = x % columns + (x / rows / columns) * columns;
    ...
    private Node constructChoice(Image source, Tiled cell) {
        ...
        img.setViewport(new Rectangle2D(cell.getTile().getOffsetX(), cell.getTile().getOffsetY(),
                cell.getTile().getWidth(), cell.getTile().getWidth()));
